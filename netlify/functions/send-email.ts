...
const DEFAULT_CONTACT_EMAIL = 'office@karailesno.bg';

const parseRecipientList = (value: string | undefined): string[] => {
  if (typeof value !== 'string') {
    return [];
  }
  ...
  const pieces = value
    .split(/[,;\n]/)
    .map((entry) => normalizeEnvValue(entry))
...
const CONTACT_RECIPIENTS = (() => {
  ...
  return recipients.length > 0 ? recipients : [DEFAULT_CONTACT_EMAIL];
})();

const CONTACT_EMAIL = CONTACT_RECIPIENTS[0];

const registerRecipient = (map: Map<string, string>, address: string | undefined): void => {
...
export const handler = async (event: NetlifyEvent): Promise<NetlifyResponse> => {
  ...
  try {
    ...
    for (const recipient of CONTACT_RECIPIENTS) {
      registerRecipient(recipientMap, recipient);
    }
    registerRecipient(recipientMap, transportConfig?.user);
    const allRecipients = Array.from(recipientMap.values());
    const primaryRecipients = CONTACT_RECIPIENTS.length > 0 ? CONTACT_RECIPIENTS : [CONTACT_EMAIL];
    const bccRecipients = allRecipients.filter((address) =>
      !primaryRecipients.some((primary) => primary.toLowerCase() === address.toLowerCase())
    );
    ...
    const info = await transporter.sendMail({
      ...
      replyTo: {
        ...
        address: normalizedPayload.email
      },
    ...
    const transportMode = transportConfig?.mode ?? 'smtp';

    if (transportMode === 'smtp' && accepted.length === 0) {
      console.error('SMTP server did not accept any recipients', { info, allRecipients });
      return jsonResponse(502, { error: 'SMTP server did not accept the message' });
    }

    if (transportMode === 'smtp' && rejected.length > 0) {
      console.warn('SMTP server rejected some recipients', { rejected, allRecipients });
    }
